...
public class Quicksort {
    ...
    //
    private static void qsortSequential(final int[] arr, final int start, final int end, int granularity) {
        int left = start;
        int right = end + 1;
        ...
        if (start < right) {
            ...
            else qsortSequential(arr, start, right, granularity);
        }
        if (left < end) {
            if (end - start < granularity) {
            ...
            qsortSequential(arr, left, end, granularity);
        }
    }
    public void qsortParallel (final int[] arr, final int start, final int end, int granularity) {
             
    }
    public static void qsortSequential(final int[] arr) {
        qsortSequential(arr, 0, arr.length-1, 700);
    }
